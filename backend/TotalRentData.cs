...
namespace RentSkiColumbia
{
    public class TotalRentData
    {
        ...
        public void Delete_order_from_listOrder(Order toDelete)
        {
            totalOrder.Remove(toDelete);

            int month = toDelete.FlightDate.Month;
            int day = toDelete.FlightDate.Day;

            Delete_from_listOrder(day, month, false, toDelete);

            day = toDelete.ReturnDate.Day;
            month = toDelete.ReturnDate.Month;

            Delete_from_listOrder(day, month, true, toDelete);
        }
        private void Delete_from_listOrder(int day, int month, bool isReturentDay, Order toDelete)
        {
            if (month == 12)
            {
                if (isReturentDay)
                {
                    monthArray[0].Days[day].TodayOrdersBack.Remove(toDelete);
                }
                else
                {
                    monthArray[0].Days[day].TodayOrdersTaking.Remove(toDelete);
            ...
            else
            {
                if (isReturentDay)
                {
                    monthArray[month].Days[day].TodayOrdersBack.Remove(toDelete);
                }
                else
                {
                    monthArray[month].Days[day].TodayOrdersTaking.Remove(toDelete);
                }
            }
        }
